...
server {
    listen 10000 ssl;
    listen [::]:10000 ssl;
    server_name {{prompt.server.public_ip}};
...
```bash
curl -I --resolve {{prompt.server.public_ip}}:10000:127.0.0.1 \
  https://{{prompt.server.public_ip}}:10000
```
...
```text
http://{{prompt.server.public_ip}}:8080
```
...
```text
http://{{prompt.server.public_ip}}:8081
```
...
1. 明确当前操作的是新服务器 `{{prompt.server.public_ip}}`；
2. 查看磁盘、内存、容器和端口状态；
...
```text
你正在维护我的 Ubuntu 22.04 服务器 {{prompt.server.public_ip}}。请把
